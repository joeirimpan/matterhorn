...
data Image =
    ...
    -- | An inline image encoded as raw terminal bytes (e.g. Sixel).
    -- The image occupies the specified number of columns and rows in
    -- the character grid. The raw bytes are emitted directly to the
    -- terminal at the appropriate cursor position.
    | SixelImage
      { sixelData :: BS.ByteString
      -- ^ The raw bytes to emit (e.g. Sixel escape sequence).
      , outputWidth :: Int
      -- ^ The number of display columns the image occupies.
      , outputHeight :: Int
      -- ^ The number of display rows the image occupies.
      }
    -- | The empty image
    --
    ...
    where
        ...
        pp i (Crop {croppedImage, leftSkip, topSkip, outputWidth, outputHeight})
            = "Crop(" ++ show leftSkip ++ "," ++ show topSkip ++ "," ++ show outputWidth ++ "," ++ show outputHeight ++ ")\n"
              ++ go (i+1) croppedImage
        pp _ (SixelImage {outputWidth, outputHeight})
            = "SixelImage(" ++ show outputWidth ++ "," ++ show outputHeight ++ ")"
...
instance NFData Image where
    ...
    rnf (SixelImage d w h) = d `deepseq` w `seq` h `seq` ()
...
imageWidth SixelImage { outputWidth = w } = w
...
imageHeight SixelImage { outputHeight = h } = h
