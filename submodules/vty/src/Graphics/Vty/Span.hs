...
data SpanOp =
    ...
    -- | A span of raw terminal bytes (e.g. Sixel image data) that
    -- occupies the given number of display columns. The bytes are
    -- emitted directly to the terminal.
    | SixelSpan
      { sixelSpanData :: !BS.ByteString
      , sixelSpanOutputWidth :: !Int
      }
...
splitOpsAt = splitOpsAt'
    where
        ...
        splitOpsAt' remainingColumns ops = case Vector.head ops of
            ...
            s@(SixelSpan _ w) -> if remainingColumns >= w
...
instance Show SpanOp where
    ...
    show (SixelSpan _ ow) = "SixelSpan(" ++ show ow ++ ")"
...
spanOpsAffectedColumns inOps = Vector.foldl' spanOpsAffectedColumns' 0 inOps
    where
        ...
        spanOpsAffectedColumns' t (SixelSpan _ w) = t + w
...
spanOpHasWidth (SixelSpan _ ow) = Just (ow, ow)
...
columnsToCharOffset cx (SixelSpan _ _) = cx
