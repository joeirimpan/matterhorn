...
-- | Make an 'Image' from raw terminal bytes (e.g. a Sixel escape
-- sequence). The bytes are emitted directly to the terminal at the
-- image's position. The caller must specify the display width and
-- height in character cells that the image will occupy.
--
-- The raw bytes should include any required escape sequences (e.g.
-- Sixel @DCS q ... ST@). No validation is performed on the data.
--
-- For inline emoji, typically @widthCells = 2@ and @heightCells = 1@.
sixelImage :: BS.ByteString
           -- ^ Raw terminal bytes (e.g. Sixel data)
           -> Int
           -- ^ Width in display columns
           -> Int
           -- ^ Height in display rows
           -> Image
sixelImage _ 0 _ = EmptyImage
sixelImage _ _ 0 = EmptyImage
sixelImage d w h = SixelImage d w h
