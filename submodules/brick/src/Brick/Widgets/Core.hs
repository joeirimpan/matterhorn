...
-- | Build a widget from raw terminal bytes (e.g. a Sixel image).
-- The bytes are emitted directly to the terminal at the widget's
-- position. The widget occupies the specified number of columns and
-- rows in the character grid.
rawTermBytes :: BS.ByteString
             -- ^ Raw terminal bytes (e.g. Sixel escape sequence)
             -> Int
             -- ^ Width in display columns
             -> Int
             -- ^ Height in display rows
             -> Widget n
rawTermBytes d w h = raw (V.sixelImage d w h)
